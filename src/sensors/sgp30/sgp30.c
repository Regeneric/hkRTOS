...
u32 SGP30_Init(I2C_Config_t* i2c, SGP30_Config_t* config) {
    HTRACE("sgp30.c -> SGP30_Init(I2C_Config_t*, SGP30_Config_t*):u32");
...
void SGP30_InitRead(I2C_Config_t* i2c, SGP30_Config_t* config) {
    ...
    if(SGP30_WriteCommand(i2c, config, hkSGP_START_MEASURE) != 2) {
...
void SGP30_InitGetBaseline(I2C_Config_t* i2c, SGP30_Config_t* config) {
    ...
    if(SGP30_WriteCommand(i2c, config, hkSGP_GET_BASELINE) != 2) {
        HWARN("SGP30_GetBaseline(): Could not init getting baseline from sensor");
        return;
    }
}
